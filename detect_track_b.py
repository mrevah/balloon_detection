...
"""
import balloon_segmetation as seg
import argparse
import os
import sys
...
def run(
        ...
        imgsz=(960, 720),  # inference size (height, width)
    ...
    for j in range(10):            
        bbox_list = []
        img = np.frombuffer(data, dtype='uint8', count=720*960*3)
        img = img.reshape((960, 720, 3))
        img = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)
        
        im0s = np.flipud(img)
        im = letterbox(im0s, imgsz, stride=stride)[0]
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for track in tracker.tracks:
                    ...
                    # matan find contour and center of the balloon usin openCV
                    """
                    image_for_segmentation = im0[int(bbox[1]):int(bbox[3]),int(bbox[0]):int(bbox[2])]
                    segmented_image , radius , circle_center = seg.find_ballon_segmentation_and_params(image_for_segmentation)
                    im0[int(bbox[1]):int(bbox[3]),int(bbox[0]):int(bbox[2])] = segmented_image
                    """
                    # matan find contour and center of the balloon usin openCV
                    ...
                    with open(txt_path + '.txt', 'a') as f:
                        ...
                        f.write(line) 
                    """
                    bbox_list.append((track.track_id, int(bbox[0]), int(bbox[1]), int(bbox[2]- bbox[0]), int(bbox[3]-bbox[1])))
